...
- `android/` — the Android app (D52): a dependency-free Java WebView shell that execs the pure-Go android/arm64 binary shipped inside the APK as `libwudict.so`. `make android-go` rebuilds the Go side, `make apk` the whole app; CI signs via repo secrets. The port adds no Go code and no build tags.
