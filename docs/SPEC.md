# gonow-dict — Architecture Spec (v0.2, Phase 0)
...
## 2. Canonical text DB (`<library folder>/text.db`, gonow-dict format v1)
...
- Triggered: CLI `gonow-dict ingest [--full] <path|dir>`; web UI per-dict "enable full-text search" / "index all" with SSE progress; headword-level indexing happens silently on first search (D13).
...
- **The library** is `store.DefaultDBDir()` (DB_DIR): gonow-dict's own working area, holding one **folder per prepared dictionary** — `<db dir>/<source file name>/{text.db, media.db, info.txt}` (D20). A dictionary is one folder: copyable, zippable, transferable; dropped into a dictionary folder it is discovered by its `text.db`.
